...
function beepBoxPitchToMakeCodeNoteWithVersion(
  pitch: number,
  octave: number,
  beepBoxVersion: number,
): number {
  if (beepBoxVersion >= 9) {
    return pitch - 35
  }
...
function createInstrumentBytes(octave: number = 4): number[] {
  return [
    ...
    0x0a,
    0x00, // [1-2] amp attack (10)
    0x64,
    0x00, // [3-4] amp decay (100)
    0xf4,
    0x01, // [5-6] amp sustain (500)
    0x64,
    0x00, // [7-8] amp release (100)
    0x00,
    0x04, // [9-10] amp amp (1024 in little-endian)
    0x00,
    0x00, // [11-12] pitch attack (0)
    0x00,
    0x00, // [13-14] pitch decay (0)
    0x00,
    0x00, // [15-16] pitch sustain (0)
    0x00,
    0x00, // [17-18] pitch release (0)
    0x00,
    0x00, // [19-20] pitch amp (0)
    0x00, // [21] amp lfo freq (0)
    0x00,
    0x00, // [22-23] amp lfo amp (0)
    0x05, // [24] pitch lfo freq (5)
    0x00,
    0x00, // [25-26] pitch lfo amp (0)
...
const MAKECODE_PRESET_INSTRUMENT_BYTES: Record<number, number[]> = {
  0: [
    0x01, 0x0a, 0x00, 0x64, 0x00, 0xf4, 0x01, 0x64, 0x00, 0x00, 0x04, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x05, 0x00, 0x00, 0x04,
  ],
  1: [
    0x0f, 0x05, 0x00, 0x12, 0x02, 0xc1, 0x02, 0xc2, 0x01, 0x00, 0x04, 0x05,
    0x00, 0x28, 0x00, 0x00, 0x00, 0x64, 0x00, 0x28, 0x00, 0x03, 0x05, 0x00,
    0x06, 0x00, 0x00, 0x04,
  ],
  2: [
    0x0c, 0x96, 0x00, 0x64, 0x00, 0x6d, 0x01, 0x90, 0x01, 0x00, 0x04, 0x78,
    0x00, 0x2c, 0x01, 0x00, 0x00, 0x64, 0x00, 0x32, 0x00, 0x00, 0x78, 0x00,
    0x0a, 0x01, 0x00, 0x05,
  ],
}
...
function getInstrumentBytesForTrack(
  trackId: number,
...
const MAKECODE_PRESET_INSTRUMENT_PARAMS: Record<
  number,
  {
    waveform: number
    octave: number
 ...
 */
function convertPatternToNoteEvents(
  pattern: BeepBoxPattern,
  octave: number,
  beepBoxVersion: number,
  tickScale: number,
): MakeCodeNoteEvent[] {
  ...
  for (const note of pattern.notes) {
    ...
    const makeCodeNotes = note.pitches.map((pitch) =>
      beepBoxPitchToMakeCodeNoteWithVersion(pitch, octave, beepBoxVersion),
    )
 ...
 */
function convertChannelToTrack(
  channel: BeepBoxChannel,
  beepBoxSong: BeepBoxSong,
  trackId: number,
): number[] {
  ...
  let currentTick = 0
  const ticksPerBar =
    beepBoxSong.beatsPerBar * beepBoxSong.ticksPerBeat * tickScale
...
export function convertBeepBoxToMakeCode(beepBoxSong: BeepBoxSong): string {
  ...
  for (
    let channelIndex = 0;
    channelIndex < beepBoxSong.channels.length;
    channelIndex++
  ) {
    ...
    if (track.length > 0) {
      ...
      const noteByteLength =
        track[noteLengthOffset] | (track[noteLengthOffset + 1] << 8)
  ...
  const computedMeasures =
    maxEndTick > 0 ? Math.ceil(maxEndTick / ticksPerMeasure) : 1
...
export function generatePasteJSON(
  beepBoxSong: BeepBoxSong,
  songName: string = 'convertedSong',
